...
import { AP_MLAS, normalizeConstituencyKey } from '../data/apMLAs';
import { AP_MPS } from '../data/apMPs';
...
const ProvisionModal = ({ entity, kind, existingUser, onClose, onSaved }) => {
  ...
  const defaultEmail =
    kind === 'mp'
      ? `${slugify(entity.lsId || entity.lsName)}.mp@tdp.local`
      : `${slugify(entity.constituency)}@tdp.local`;
  const defaultName =
    kind === 'mp'
          ...
          {!isEdit && kind === 'mla' && (
            <div>
              ...
              >
                ...
                <option value="nara_lokesh">Nara Lokesh (special)</option>
...
const ConstituencyLogins = () => {
  ...
  const totalMla = AP_MLAS.length;
  const totalMp = AP_MPS.length;
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 max-w-2xl">
              One login per Andhra Pradesh assembly constituency and per Lok Sabha seat. Scoped
