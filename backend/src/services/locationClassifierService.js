 ...
 * ─────────────────────────────────────────────────────────────────────
 * RapidAPI-backed Andhra Pradesh location classifier. Given the raw text
 * of a social-media post (plus optional metadata), returns the most
 * relevant AP constituency / district / Lok-Sabha seat so the post can
 * be auto-routed to the right MLA / MP dashboard via the existing
 * scopeMiddleware (`detected_location.constituency`).
 *
 * Output (or null if the post is not about AP):
 *   {
 *     constituency: '<exact name from ap_mlas.json>',
...
const CONSTITUENCIES = [...new Set(
    AP_MLAS.map((m) => String(m.constituency || '').trim()).filter(Boolean)
)];
...
const heuristicLookup = (text) => {
    ...
    // "TIRUPATI RURAL" wins over "TIRUPATI" when both appear.
...
const buildPrompt = (text, ctx) => `You are a geo-routing classifier for an Andhra Pradesh political-monitoring system.
Your one job: given a social-media post, decide which AP assembly constituency it is most likely about.
...
Decision rules:
  ...
  3. If the post is about a known MLA / MP by name and you know their seat
     — pick that seat.
  4. If the post is about Andhra Pradesh broadly with no specific seat,
     pick "NONE" — do not guess a seat at random.
  5. If the post is not about Andhra Pradesh at all, pick "NONE".
  6. NEVER invent a constituency name. NEVER pick a Telangana / other-state
     seat. NEVER pick more than one.
...
/**
 * Classify a post to its AP constituency. Returns null if the post is
 * not confidently about any AP seat.
 */
