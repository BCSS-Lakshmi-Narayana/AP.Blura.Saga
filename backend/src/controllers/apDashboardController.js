...
const { isAndhraPradeshLocation, AP_DISTRICTS } = require('../config/andhraPradeshLocations');
  ...
  try {
    ...
    const [
      ...
      Grievance.distinct('detected_location.constituency', { ...curMatch, 'detected_location.constituency': { $ne: null, $ne: '' } })
        .then(arr => arr.filter(c => c && isAndhraPradeshLocation(c)).length),
...
const getAPDistrictPerformance = async (req, res) => {
  try {
    ...
    const districts = rows
      .filter(r => r._id && isAndhraPradeshLocation(r.district || r._id))
      .map(r => ({
...
const getAPMapData = async (req, res) => {
  try {
    ...
    const toMap = (rows, apFilter) => {
      const m = {};
      rows.forEach(r => {
        if (!r._id || !isAndhraPradeshLocation(r.name || r._id)) return;
...
const getAPAiInsights = async (req, res) => {
  try {
    ...
    const apDistricts = districtRows.filter(r => isAndhraPradeshLocation(r.district));
...
ANDHRA PRADESH POLITICAL WATCH — INTELLIGENCE SUMMARY
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [{
            role: 'system',
            content: 'You are a political intelligence analyst for Andhra Pradesh, India. Analyze the provided data and generate 5 specific, actionable insights. Each insight must be based directly on the numbers provided. Be concise (1-2 sentences each). Focus on trends, anomalies, and opportunities for TDP leadership. Output as a JSON array of strings.'
          }, {
            role: 'user',
    ...
    if (insights.length === 0 && process.env.GEMINI_API_KEY) {
      try {
        ...
        const result = await model.generateContent({
          contents: [{ role: 'user', parts: [{ text: `${analyticsContext}\n\nYou are a political intelligence analyst for Andhra Pradesh, India. Generate exactly 5 specific, data-driven insights based on the analytics above. Each insight must reference actual numbers. Focus on trends, anomalies, and implications for TDP leadership. Return ONLY a valid JSON object: {"insights": ["insight1", "insight2", "insight3", "insight4", "insight5"]}` }] }]
        });
...
const getAPGeoHighlights = async (req, res) => {
  try {
    ...
    const apRows = rows.filter(r => isAndhraPradeshLocation(r.district || r._id));
