...
const glanceSearch = async (req, res) => {
    try {
        ...
        const knownKeywords = ['amaravati', 'vijayawada', 'visakhapatnam', 'vizag', 'guntur', 'tirupati', 'nellore', 'kurnool', 'kakinada',
            'andhra', 'andhra pradesh', 'telugu', 'karnataka', 'maharashtra', 'tamil', 'kerala', 'odisha', 'rayalaseema',
            'political', 'government', 'parliament', 'election', 'minister', 'chief', 'police', 'congress', 'tdp', 'ysrcp', 'jsp',
        ...
        // Scope filter: Andhra Pradesh-only focus (primary), police + politics/government (primary).
        const topicRegex = /\b(police|ap\s*police|dgp|commissioner|fir|arrest|detain|detention|raid|crime|murder|assault|rape|molest|kidnap|law\s*and\s*order|public\s*safety|security|traffic|enforcement|checkpoint|protest|rally|stone\s*pelting|clash|communal|riot|court|hc|high\s*court|sc|supreme\s*court|bail|sit|ed|cbi|acb|vigilance|municipal|collector|rdo|tahsildar|minister|mla|mp|cm|chief\s*minister|governor|cabinet|budget|assembly|parliament|election|polls|by\s*election|vote|evm|tdp|ysrcp|ycp|jsp|janasena|congress|chandrababu|naidu|cbn|nara\s*lokesh|lokesh|pawan\s*kalyan|jagan|jagan\s*mohan\s*reddy|water|power|electricity|road|transport|metro|bus|rtc|drainage|flood|rain|weather|pollution|health|hospital|school|college|education|university|jobs|employment|student|farmer|agriculture)\b/i;

        // Andhra Pradesh signals: state + major cities/districts/areas.
        const locRegex = /\b(andhra\s*pradesh|andhra|amaravati|vijayawada|bezawada|guntur|visakhapatnam|vizag|vishakhapatnam|tirupati|tirumala|nellore|kurnool|kadapa|cuddapah|anantapur|ananthapuramu|chittoor|kakinada|rajahmundry|rajamahendravaram|eluru|ongole|srikakulam|vizianagaram|kuppam|mangalagiri|pithapuram|machilipatnam|bhimavaram|narasaraopet|tenali|chilakaluripet|proddatur|adoni|hindupur|dharmavaram|tadepalligudem|gudivada|nandyal|tadipatri|chirala|rayalaseema|godavari|krishna\s*district|prakasam|nandhyal)\b/i;
