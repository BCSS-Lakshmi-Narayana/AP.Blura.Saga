...
const createDefaultUsers = async () => {
  try {
    const defaultUsers = [
      {
        email: process.env.DEFAULT_ADMIN_EMAIL || 'admin@apwatch.in',
        password: process.env.DEFAULT_ADMIN_PASSWORD || '#TDP@AndhraPradesh2026',
        full_name: process.env.DEFAULT_ADMIN_NAME || 'AP Political Watch Super Admin',
...
const startServer = async () => {
  ...
  app.listen(PORT, () => {
    console.log(`╔══════════════════════════════════════════════════════════╗`);
    console.log(`║  AP POLITICAL WATCH · Chandrababu Naidu · TDP Andhra Pradesh ║`);
